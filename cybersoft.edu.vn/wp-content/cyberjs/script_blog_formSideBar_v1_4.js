function getValueSideBarBlog() {
  let email = document.querySelector("#dhvc_form_control_email").value;
  let soDT = document.querySelector("#dhvc_form_control_sodienthoai").value;
  let loaiForm = document.querySelector(
    ".cyberLeftBlog .widget_text .widgettitle"
  ).innerText;
  if (email && soDT) {
    const promise = axios({
      method: "post",
      url: "https://apicrm.cybersoft.edu.vn/api/leadform",
      data: {
        hoTen: "SideBar-BlogForm",
        email,
        soDT,
        loaiForm,
      },
      headers: {
        Authorization:
          "Bearer " +
          "eyJhbGciOiJodHRwOi8vd3d3LnczLm9yZy8yMDAxLzA0L3htbGRzaWctbW9yZSNobWFjLXNoYTI1NiIsInR5cCI6IkpXVCJ9..jwNWA-hbWs2sIkfYYdcxPyQC_bQHIfdowEN77Toug2I",
      },
    });

    promise.then((result) => {});
  }
}

const getButtonSubmitSideBar = document.querySelector(
  ".cyberLeftBlog .dhvc-form-submit"
);

if (getButtonSubmitSideBar) {
  getButtonSubmitSideBar.addEventListener("click", getValueSideBarBlog);
}
